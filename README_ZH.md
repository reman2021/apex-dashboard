...
> 本项目由 [PandoraReads/apex-dashboard](https://github.com/PandoraReads/apex-dashboard) fork 后修改而来，包含额外功能优化与维护调整。
...
| 主题 | 风格 | Obsidian 基础主题 |
|------|------|-------------------|
| **默认** | 从当前 Obsidian 主题派生的低饱和优雅配色 | 浅色和深色 |
| **大地** | 泥土棕、木色与苔绿组合，温暖沉稳 | 浅色和深色 |
| **北欧** | 冰灰、峡湾蓝与冷青组合，克制清爽 | 浅色和深色 |
| **极光** | 极地夜空玻璃感，青绿、天蓝与紫光交织 | 浅色和深色 |
| **春日** | 桃粉、樱色、暖金和嫩绿，红色强调更柔和 | 浅色和深色 |
| **岛屿** | 奶油沙色、海风薄荷和森林绿标题 | 浅色和深色 |
| **苔原** | 冷灰绿、苔藓绿与霜白卡片 | 浅色和深色 |
| **花漾** | 花瓣粉、柔玫瑰和浅樱色，浅色红色强调更低饱和 | 浅色和深色 |
| **抹茶** | 抹茶奶绿与茶点般的温金 | 仅浅色 |
| **丁香** | 丁香紫、温柔灰紫与玫瑰灰 | 仅浅色 |
| **薄雾** | 蓝灰雾气、柔和银灰与玻璃暗雾感 | 仅深色 |
| **余烬** | 烟熏煤色与暖橙余烬光感 | 仅深色 |
| **翡翠** | 玉石绿、竹雾绿与温润金 | 仅深色 |
| **日食** | 石墨黑、银灰和日食阴影层次 | 仅深色 |
...
### 当前 fork 修改
- **来源说明与 manifest 修复** — 更新项目元信息，注明本项目 fork 自 PandoraReads/apex-dashboard，并修复 manifest ID 不一致导致插件无法保持启用的问题
- **Banner 名言库** — 支持填写 Vault 相对路径的名言库；每个非空行可写为 `名言 | 作者`、`名言 - 作者` 或纯名言，作者可留空
- **Banner 编辑简化** — 移除旧的“名言合集”编辑入口，随机名言统一通过“名言库路径”实现
- **背景图片列表** — 将单行背景图片路径改为背景图片列表；一张图片固定显示，多张图片约每 30 分钟轮播
- **名言颜色控件** — 名言颜色改为圆形色块选择器，并支持一键重置默认颜色
- **Todo 归档管理** — Todo 卡片支持归档、恢复和查看归档列表，归档项会显示归档日期
- **倒数日重构** — 将旧倒计时改为倒数日，最多同时显示 3 个目标日期，点击“设置日期”弹出日历，固定按剩余天数展示
- **正在阅读重构** — 将“读书记录”改为“正在阅读”，选择书籍改为手动填写书名、作者、总页数和封面地址，不再搜索书籍
- **数据库筛选配置** — 数据库配置弹窗列出用户显式写在笔记开头的属性，选择属性后显示可勾选的属性值，并按选中值筛选
- **弹窗主题同步** — 数据库配置、横幅编辑、卡片编辑、控件配置、倒数日设置等弹窗会继承当前工作台主题
- **启动打开工作台** — 新增可选设置，默认关闭，可在打开库时自动打开工作台
- **番茄钟优化** — 新增重置按钮，修复番茄统计图标刷新后消失，并修复深色模式下专注统计圆形图文字颜色不适配的问题
- **悬浮与布局修复** — 移除卡片和快捷操作项 hover 时的上移效果，固定 Todo 卡片高度，并修复增删任务时横幅闪动和轮播被触发的问题
- **工作台名称设置** — 新增工作台标签页名称设置，留空时回退默认“工作台”
- **详细中文说明** — 更新 `使用说明_ZH.md`，覆盖安装、横幅、快捷操作、番茄钟、热力图/追踪器、倒数日、正在阅读、Todo 归档、数据库筛选、主题和常见问题
